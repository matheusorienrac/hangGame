...
echo "<script> var category = ($dica)</script>"
//////////////////////////////// A VARIAVEL WORD TEM QUE CONTER A PALAVRA EM SI. PUXA DO BANCO DE DADOS
echo "<script> var word = ($linha)</script>"
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var coluna = 1;
var newCategory = "";
var newWord = "";
...
// Parte do Teclado que só tem uso na parte do jogo em si e na parte de adicionar palavras novas
var keyPressed;
	...
	if(!blocked){
		if((screen3Ready)||(screen4Ready)){ // Para a tela de jogo e de adicionar palavras, apenas letras sao aceitas
			...
			if(screen5Ready){ // Qualquer tecla digitada na tela de gameover serve para retornar a tela de menu
				...
				screen2Ready = true;
				soundOn = true;
				snd.pause(); // Pausa as musicas e poe todas elas no inicio para serem tocadas novamente
				snd.currentTime = 0;
				gamesnd.pause();
...
function drawWords(){
	...
	for(var y = 0; y < chanceword.length;y++)
		ctx.drawImage(font,(chanceword.charAt(y).charCodeAt(0) - 65)* 18,0,18,18,228 + y * 20,65,25,25)
	chanceword = "CHANCES"; // Para ele digitar o menuzim do chances
	for(var y = 0; y < chanceword.length;y++)
		ctx.drawImage(font,(chanceword.charAt(y).charCodeAt(0) - 65)* 18,0,18,18,50 + y * 22,65,25,25)
	// DESENHA A PALAVRA FINAL QUANDO O JOGO TERMINA
	if (chances == 0)
		for(var z = 0; z < word.length; z++)
				ctx.drawImage(font,(word.charAt(z).charCodeAt(0) - 65)* 18,0,18,18,(inicial) + space * (z) + 6,270,18,18);
	for(var z = 0; z < category.length; z++){
		ctx.drawImage(font,(category.charAt(z).charCodeAt(0) - 65)* 18,0,18,18,570 + (z) * 15,65,25,25);
		
	}
	for(var z = 0; z < categoryword.length; z++){
		ctx.drawImage(font,(categoryword.charAt(z).charCodeAt(0) - 65)* 18,0,18,18,415 + (z) * 15,65,25,25);
	...
	if ((mousePos.x >= 0)&&(mousePos.x <= 27)&&(mousePos.y >= 0)&&(mousePos.y <= 26 )){ // Verifica se o seu mouse está posicionaod em cima do botao de som
		if((x >= 0)&&(x <= 27)&&(y >= 0)&&(y <= 26)){ // Verifica se voce clicou nele
			if(soundOn) // Se tiver ligado, desliga
				...
				soundOn = true;
			x = 50; // Gambiarra para ele não guardar que o ultimo lugar que você clicou foi no som. Sem isso, se voce clicar uma vez no som, toda vez que seu mouse estiver em cima ele ira achar que voce clicou
